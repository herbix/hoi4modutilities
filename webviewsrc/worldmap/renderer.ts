...
export class Renderer extends Subscriber {
    ...
    constructor(
        ...
        viewModeControllers: ViewModeControllers,
    ) {
        super();
        ...
        this.addSubscription(
            combineLatest([
                ...
                topBar.viewMode$,
                topBar.colorSet$,
                ...viewModeControllers.getHoverObservables(),
                ...viewModeControllers.getSelectedObservables(),
    ...
    public renderCanvas(forceDrawMap: boolean = false): void {
        ...
        this.topBar.viewModeController.renderHoverSelection(this, this.loader.worldMap);
    ...
    public static renderMapImpl(canvas: HTMLCanvasElement, topBar: TopBar, viewPoint: ViewPoint, worldMap: FEWorldMap, otherRenderContext?: Partial<RenderContext>) {
        ...
        const renderContext: RenderContext = {
            ...
            stateToOwnerCountry: worldMap.getStateToCountryMap(topBar.selectedConditions$.value, 'owner'),
    ...
    public static getProvinceLabelColor(
        province: Province,
        labelPosition: Point,
        worldMap: FEWorldMap,
        ...
        const provinceColor = getColorByColorSet(
            renderContext.topBar.colorSet$.value,
            provinceAtLabel ?? province,
            worldMap,
            renderContext,
        );
        return toColor(getHighConstrastColor(provinceColor));
    }
    ...
    private static isEdgeVisible(topBar: TopBar, viewPoint: ViewPoint) {
        if (topBar.display.selectedValues$.value.includes('adaptzooming')) {
            const scale = viewPoint.scale;
            return topBar.viewModeController.edgeRenderScale <= scale && topBar.display.selectedValues$.value.includes('edge');
        }
    ...
    private static isLabelVisible(topBar: TopBar, viewPoint: ViewPoint) {
        if (topBar.display.selectedValues$.value.includes('adaptzooming')) {
            const scale = viewPoint.scale;
            return topBar.viewModeController.labelRenderScale <= scale && topBar.display.selectedValues$.value.includes('label');
        }

        return topBar.display.selectedValues$.value.includes('label');
    }

    public get selectedConditions(): ConditionItem[] {
        return this.topBar.selectedConditions$.value;
    }

    public isMouseHighlightVisible() {
    ...
    private static renderEdges(
        ...
        const { provinceToState, renderedProvinces, topBar, viewPoint } = renderContext;
        const scale = viewPoint.scale;
        ...
        for (const provinceEdge of province.edges) {
            ...
            if (!impassable && paths.length > 0 && !topBar.viewModeController.shouldRenderProvinceEdge(renderContext, province, provinceEdge, worldMap)) {
                continue;
            }
    ...
    public renderRegionHoverSelection(worldMap: FEWorldMap, hover: { provinces: number[] } | undefined, selected: { provinces: number[] } | undefined) {
        if (selected) {
    ...
    public renderTooltip(
        tooltip: string,
        sizeCallback?: (width: number, height: number) => {width: number, height: number},
        renderCallback?: (context: CanvasRenderingContext2D, x: number, y: number) => void,
    ) {
        const backCanvasContext = this.backCanvasContext;
        ...
        if (renderCallback) {
            renderCallback(
                ...
                cursorY + toolTipOffsetY + marginY + text.length * (fontSize + linePadding),
            );
        }
    }
...
function getColorByColorSet(
    ...
    switch (colorSet) {
        ...
        case 'controller':
            {
                if (renderContext.colorSetState === undefined) {
                    renderContext.colorSetState = worldMap.getStateToCountryMap(topBar.selectedConditions$.value, 'controller');
                }

                const stateId = provinceToState[province.id];
                const controller = stateId === undefined ? undefined : (renderContext.colorSetState[stateId] ?? stateToOwnerCountry[stateId]);
        ...
        case 'terrain':
            {
                if (renderContext.colorSetState === undefined) {
                    renderContext.colorSetState = arrayToMap(worldMap.terrains, 'name');
                }
                ...
                return (renderContext.colorSetState as Record<string, Terrain | undefined>)[navalTerrain ?? province.terrain]?.color ?? 0;
            }
        case 'continent':
            if (renderContext.colorSetState === undefined) {
                let continent = 0;
                worldMap.forEachProvince(p => (p.continent > continent ? continent = p.continent : 0, false));
                renderContext.colorSetState = avoidPowerOf2(continent + 1);
            }
            return province.continent !== 0 ? valueAndMaxToColor(province.continent + 1, renderContext.colorSetState) : defaultColor(province);
        case 'stateid':
            {
                if (renderContext.colorSetState === undefined) {
                    renderContext.colorSetState = avoidPowerOf2(worldMap.statesCount);
                }
                const stateId = provinceToState[province.id];
                return stateId !== undefined ? valueAndMaxToColor(stateId < 0 ? 0 : stateId, renderContext.colorSetState) : defaultColor(province);
            }
        case 'warnings':
            {
                const isLand = province.type === 'land';
                const applyWarningFilter = topBar.viewMode$.value === 'warnings';
                ...
                return worldMap.getProvinceWarnings(
                        !applyWarningFilter || warningFilter.includes('province') ? province : undefined,
                        !applyWarningFilter || warningFilter.includes('state') ? state : undefined,
                        !applyWarningFilter || warningFilter.includes('strategicregion') ? strategicRegion : undefined,
                        !applyWarningFilter || warningFilter.includes('supplyarea') ? supplyArea : undefined
                    ).length > 0 ?
        ...
        case 'manpower':
            {
                ...
                if (renderContext.colorSetState === undefined) {
                    ...
                    renderContext.colorSetState = maxManpower;
                }

                const stateId = provinceToState[province.id];
                const state = worldMap.getStateById(stateId);
                const value = manpowerHandler(state?.manpower ?? 0) / manpowerHandler(renderContext.colorSetState);
        ...
        case 'victorypoint':
            {
                if (renderContext.colorSetState === undefined) {
                    ...
                    renderContext.colorSetState = maxVictoryPoint;
                }
        ...
        case 'resources':
            {
                ...
                if (renderContext.colorSetState === undefined) {
                    ...
                    renderContext.colorSetState = maxResources;
                }

                const stateId = provinceToState[province.id];
                const state = worldMap.getStateById(stateId);
                const numResources = state ? Object.values(state.resources).reduce<number>((p, c) => p + (c ?? 0), 0) : 0;
                const value = resourcesHandler(numResources) / resourcesHandler(renderContext.colorSetState);
                return valueToColorGYR(value);
            }
        case 'strategicregionid':
            {
                if (renderContext.colorSetState === undefined) {
                    renderContext.colorSetState = avoidPowerOf2(worldMap.strategicRegionsCount);
                }
                const strategicRegionId = provinceToStrategicRegion[province.id];
                return valueAndMaxToColor(strategicRegionId === undefined || strategicRegionId < 0 ? 0 : strategicRegionId, renderContext.colorSetState);
            }
        case 'supplyareaid':
            {
                if (renderContext.colorSetState === undefined) {
                    renderContext.colorSetState = avoidPowerOf2(worldMap.supplyAreasCount);
                }
                const stateId = provinceToState[province.id];
                const supplyAreaId = stateId !== undefined ? stateToSupplyArea[stateId] : undefined;
                return supplyAreaId !== undefined ? valueAndMaxToColor(supplyAreaId < 0 ? 0 : supplyAreaId, renderContext.colorSetState) : defaultColor(province);
            }
        case 'supplyvalue':
            {
                if (province.type === 'sea') {
                ...
                if (renderContext.colorSetState === undefined) {
                    ...
                    renderContext.colorSetState = maxSupplyValue;
                }

                const stateId = provinceToState[province.id];
                const supplyAreaId = stateId ? stateToSupplyArea[stateId] : undefined;
                const supplyArea = worldMap.getSupplyAreaById(supplyAreaId);
                const value = (supplyArea?.value ?? 0) / (renderContext.colorSetState);
                return valueToColorGYR(value);
            }
        default:
